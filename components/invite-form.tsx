...
export function InviteForm({ projects }: { projects: { id: string; name: string; siteId: string }[] }) {
  ...
  return (
    <section className="mb-6 rounded-lg border border-black/10 bg-white p-4 shadow-sm">
        ...
        onSubmit={(event) => {
          ...
          startTransition(async () => {
            try {
              ...
              setError(caught instanceof Error ? caught.message : t("pages.settings.couldNotCreateInvite"));
            }
          });
        }}
      >
        <input name="email" type="email" placeholder={t("pages.settings.inviteEmail")} required className="rounded-md border px-3 py-3" />
        ...
        <button className="rounded-md bg-ink px-4 py-3 font-semibold text-white" disabled={pending}>{pending ? t("common.actions.creating") : t("common.actions.createInvite")}</button>
        {role === "PROJECT_MANAGER" ? (
          <div className="rounded-md border border-black/10 p-3 md:col-span-4">
            <p className="mb-2 text-sm font-semibold"><T k="pages.settings.assignedProjects" /></p>
      ...
      {link ? <p className="mt-4 rounded-md bg-field p-3 text-sm font-medium"><T k="pages.settings.inviteLink" />: <a className="text-mint" href={link}>{link}</a></p> : null}
    </section>
  );
}
