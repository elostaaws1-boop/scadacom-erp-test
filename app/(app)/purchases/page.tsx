          ...
          return (
            <section className="rounded-lg border border-black/10 bg-white p-5 shadow-sm" key={item.id}>
              <div className="flex flex-col justify-between gap-3 md:flex-row">
                <div><h2 className="font-semibold">{item.item} · {mad(item.amount)}</h2><p className="text-sm text-stone-500">{item.project.name} · {item.category} · {item.paymentMethod} · {item.receipts.length} receipt{item.receipts.length === 1 ? "" : "s"} · receipt total {mad(receiptTotal)}</p></div>
