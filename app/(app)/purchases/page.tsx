...
export default async function PurchasesPage() {
  ...
  const projects = await prisma.project.findMany({
    where: { id: { in: Array.from(new Set(purchases.map((item) => item.projectId))) } },
          ...
          return (
            <section className="rounded-lg border border-black/10 bg-white p-5 shadow-sm" key={item.id}>
              <div className="flex flex-col justify-between gap-3 md:flex-row">
                <div>
                  <h2 className="font-semibold">{item.item} / {mad(item.amount)}</h2>
                  <p className="text-sm text-stone-500">
                    {projectNames.get(item.projectId) ?? "Missing project"} / {item.category} / {item.paymentMethod} / {item.receipts.length} receipt{item.receipts.length === 1 ? "" : "s"} / receipt total {mad(receiptTotal)}
                  </p>
                </div>
                <StatusBadge status={item.status} />
              </div>
              {item.status === "PENDING" ? <div className="mt-4"><ApprovalForm entity="purchase" id={item.id} amount={Number(item.amount)} /></div> : null}
            </section>
          );
        })}
      </div>
